...
壓縮档案 = zipfile.ZipFile("20210815A原.zip", mode="w")
壓縮档案.write("result.csv")
壓縮档案.close()

提交表 = 預測表.loc[:, ["文章", "預測"]]
提交表.loc[提交表.文章.isin([1691287, 1691582, 1690681, 1690600, 1660725, 1690670, 1691389, 1643275]), "預測"] = 36
提交表.columns = ["article_id", "orders_3h_15h"]
提交表.to_csv("result.csv", index=False)
壓縮档案 = zipfile.ZipFile("20210815ABzip", mode="w")
壓縮档案.write("result.csv")
壓縮档案.close()
